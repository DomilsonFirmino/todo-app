...
export default function TodoComponent({styles,Todo, handleCheck, handleDelete}:{Todo:TodoType, handleCheck: ()=> void,styles?: string, handleDelete: ()=> void}) {
  return (
    <div className={`flex justify-between gap-4 p-4 ${styles} Todo`}>
      <div className={`flex gap-4 items-center`}>
        ...
        <p className={`break-all text-gray-50 font-semibold ${Todo.status === "completed" && "line-through"}`}>{Todo.description}</p>
